...
export function requireAuthentication(req: Request, res: Response, next: NextFunction) {
  return passport.authenticate('jwt', { session: false }, (err: Error, user: users, info: any) => {
    if (err) {
      return next(err);
    }
...
// export const getAdminDetails = [requireAdminAuthentication, getAdminDetailsHandler];
...
async function getUsersHandler(req: Request, res: Response): Promise<Response> {
  try {
    ...
    if (page < totalPages) {
      links.next = `${req.protocol}://${req.get('host')}${req.path}?page=${page + 1}`;
    }
    if (page > 1) {
      links.prev = `${req.protocol}://${req.get('host')}${req.path}?page=${page - 1}`;
    }
...
export const getUser = [requireAuthentication, userIdDoesExistValidator, validationCheck, getUserHandler];
...
export const deleteUser = [userIdDoesExistValidator, validationCheck, deleteUserHandler];
...
async function patchUserHandler(req: Request, res: Response) {
  ...
  const userToCreate = await prisma.user.update({
    where: {
      id,
    },
    data: {
