...
export async function getUserById(id: number): Promise<user | null> {
  ...
  try {
    userToSearch = await prisma.user.findFirst({
      where: { id },
    });
  } catch {
    return null;
...
export const xssSanitizer = (parameter: string) => body(parameter).customSanitizer((v) => xss(v));

export const xssSanitizerMany = (params: string[]) => params.map((parameter) => xssSanitizer(parameter));

export const genericSanitizer = (parameter: string) => body(parameter).trim().escape();

export const genericSanitizerMany = (params: string[]) => params.map((parameter) => genericSanitizer(parameter));

export const stringValidator = ({ field = '', valueRequired = true, maxLength = 0, optional = false } = {}) => {
  const val = body(field)
    ...
    .withMessage(
      [field, valueRequired ? 'required' : '', maxLength ? `max ${maxLength} characters` : '']
        .filter((i) => Boolean(i))
        .join(' ')
    );
...
// export const departmentDoesNotExistValidator = body('title').custom(async (title) => {
...
// export const courseTitleDoesNotExistValidator = body('title').custom(async (title) => {
...
export const userNameDoesNotExistValidator = body('username').custom(async (username) => {
  if (await getUserByName(username)) {
...
export const userNameDoesExistValidator = body('username').custom(async (username) => {
  if (await getUserByName(username)) {
    return Promise.resolve();
  }
  return Promise.reject(new Error('A user with this username already exists'));
...
// export const courseIdDoesNotExistValidator = body('courseId').custom(async (courseId) => {
...
// export const itemNameDoesNotExistValidator = body('name').custom(async (name) => {
...
// export const categoryIdDoesExistValidator = body('categoryId').custom(async (id) => {
...
// export const categoryNameDoesNotExistValidator = body('name').custom(async (name) => {
//   if (!(await getCategoryByName(name))) {
//     return Promise.resolve();
//   }
//   return Promise.reject(new Error('category with name already exists'));
...
// export const categoryIdParamDoesExistValidator = param('categoryId').custom(async (id) => {
//   if (!(await getCategoryById(Number.parseInt(id, 10)))) {
//     return Promise.reject(new Error('category with id does not exist'));
//   }
//   return Promise.resolve();
...
// export const questionIdDoesExistValidator = param('questionId').custom(async (id) => {
...
// export const userHasNotVotedForQuestionValidator = param().custom(async (values, { req }) => {
//   console.log(values);
...
//   if (questionToSearch.firstItemId !== itemId && questionToSearch.secondItemId !== itemId) {
//     return Promise.reject(new Error('item with id does not exist in this question'));
//   }
//   const { user } = req;
//   if (
...
//   ) {
//     return Promise.reject(new Error('User has already voted for this question'));
//   }
